...
func Seed(ctx context.Context, db *ent.Client) error {
	...
	// count, err := db.Accounting.Query().Count(ctx)
	// if err != nil || count > 0 {
	//     return err
	// }
	// _, err = db.Accounting.Create().SetName("Sample Accounting").Save(ctx)
	// return err
...
var AccountGroups = []GroupSeed{
	...
	{"LAND", "Land & Building", acct_group.NatureASSET, "FIX_AST"},
...
func SeedAccountGroups(ctx context.Context, client *ent.Client) error {
	...
	groupMap := make(map[string]*ent.Acct_Group)
	...
	for _, g := range AccountGroups {
		...
		if g.ParentCode != "" {
			...
			if !ok {
				return fmt.Errorf(
					"parent group '%s' not found for '%s'",
					g.ParentCode,
					g.Code,
				)
			}
...
func SeedLedgers(ctx context.Context, client *ent.Client) error {
	...
	for _, l := range Ledgers {
		...
		if !ok {
			return fmt.Errorf(
				"group '%s' not found for ledger '%s'",
				l.GroupCode,
				l.Code,
			)
		}
