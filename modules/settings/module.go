...
import (
	...
	"goapp/internal/core"
	"goapp/modules/settings/handler"
...
func New(c *core.Container) *Module {
	...
	if err := m.LoadMeta(); err != nil {
		panic("settings module: " + err.Error())
	}
	return m
}
...
func (m *Module) Init(_ context.Context) error {
	m.Logger.Info("init", "module", "settings")
	return nil
}
...
func (m *Module) PostRegister(ctx context.Context, deps map[string]core.Module) error {
	m.Logger.Info("post-register", "module", "settings")

	// Load all settings from DB into Store
	if err := m.service.Load(ctx); err != nil {
		return err
	}
	...
	// Other modules can subscribe to "settings.ready" to react.
	return m.deps.EventBus.Publish(ctx, core.Event{
		Topic:   "settings.ready",
		Payload: map[string]string{"module": "settings"},
		Source:  "settings",
	})
}

func (m *Module) Shutdown(ctx context.Context) error {
	m.Logger.Info("shutdown", "module", "settings")
	...
	//   case <-done:
	//   case <-ctx.Done():
	//       m.Logger.Warn("shutdown timeout, buffer not fully flushed", "module", "settings")
	//   }
	// ─────────────────────────────────────────────────────────────────────────
