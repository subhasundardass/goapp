...
// The AccountingFunc type is an adapter to allow the use of ordinary
// function as Accounting mutator.
type AccountingFunc func(context.Context, *ent.AccountingMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AccountingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AccountingMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AccountingMutation", m)
}
