...
type Client struct {
	...
	// Accounting is the client for interacting with the Accounting builders.
...
func (c *Client) init() {
	...
	c.Accounting = NewAccountingClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		ctx:          ctx,
		config:       cfg,
		Accounting:   NewAccountingClient(cfg),
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		c.Accounting, c.Acct_Group, c.Country, c.Example, c.Journal, c.Journal_Line,
		c.Ledger, c.PartyMaster, c.Setting, c.State, c.Test,
	} {
		n.Use(hooks...)
	}
}
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	switch m := m.(type) {
	case *AccountingMutation:
		return c.Accounting.mutate(ctx, m)
...
// AccountingClient is a client for the Accounting schema.
type AccountingClient struct {
	config
}

// NewAccountingClient returns a client for the Accounting from the given config.
func NewAccountingClient(c config) *AccountingClient {
	return &AccountingClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `accounting.Hooks(f(g(h())))`.
func (c *AccountingClient) Use(hooks ...Hook) {
	c.hooks.Accounting = append(c.hooks.Accounting, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `accounting.Intercept(f(g(h())))`.
func (c *AccountingClient) Intercept(interceptors ...Interceptor) {
	c.inters.Accounting = append(c.inters.Accounting, interceptors...)
}

// Create returns a builder for creating a Accounting entity.
func (c *AccountingClient) Create() *AccountingCreate {
	mutation := newAccountingMutation(c.config, OpCreate)
	return &AccountingCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of Accounting entities.
func (c *AccountingClient) CreateBulk(builders ...*AccountingCreate) *AccountingCreateBulk {
	return &AccountingCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *AccountingClient) MapCreateBulk(slice any, setFunc func(*AccountingCreate, int)) *AccountingCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &AccountingCreateBulk{err: fmt.Errorf("calling to AccountingClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*AccountingCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
// Update returns an update builder for Accounting.
func (c *AccountingClient) Update() *AccountingUpdate {
	mutation := newAccountingMutation(c.config, OpUpdate)
	return &AccountingUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *AccountingClient) UpdateOne(_m *Accounting) *AccountingUpdateOne {
	mutation := newAccountingMutation(c.config, OpUpdateOne, withAccounting(_m))
	return &AccountingUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *AccountingClient) UpdateOneID(id int) *AccountingUpdateOne {
	mutation := newAccountingMutation(c.config, OpUpdateOne, withAccountingID(id))
	return &AccountingUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for Accounting.
func (c *AccountingClient) Delete() *AccountingDelete {
	mutation := newAccountingMutation(c.config, OpDelete)
	return &AccountingDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *AccountingClient) DeleteOne(_m *Accounting) *AccountingDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *AccountingClient) DeleteOneID(id int) *AccountingDeleteOne {
	builder := c.Delete().Where(accounting.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &AccountingDeleteOne{builder}
}

// Query returns a query builder for Accounting.
func (c *AccountingClient) Query() *AccountingQuery {
	return &AccountingQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAccounting},
		inters: c.Interceptors(),
	}
}

// Get returns a Accounting entity by its id.
func (c *AccountingClient) Get(ctx context.Context, id int) (*Accounting, error) {
	return c.Query().Where(accounting.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AccountingClient) GetX(ctx context.Context, id int) *Accounting {
...
func (c *AccountingClient) Hooks() []Hook {
	return c.hooks.Accounting
}

// Interceptors returns the client interceptors.
func (c *AccountingClient) Interceptors() []Interceptor {
	return c.inters.Accounting
}

func (c *AccountingClient) mutate(ctx context.Context, m *AccountingMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AccountingCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AccountingUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AccountingUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AccountingDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Accounting mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		Accounting, Acct_Group, Country, Example, Journal, Journal_Line, Ledger,
		PartyMaster, Setting, State, Test []ent.Hook
	}
	inters struct {
		Accounting, Acct_Group, Country, Example, Journal, Journal_Line, Ledger,
		PartyMaster, Setting, State, Test []ent.Interceptor
	}
)
